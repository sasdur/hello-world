    ...
    stages {
		...
		stage ('SSH WAR and DockerFile to TomcatServer' ){
			...
			steps {
					...
					'''cd /opt/docker
						...
						''', execTimeout: 120000, flatten: false, makeEmptyDirs: false, noDefaultExcludes: false, patternSeparator: '[, ]+', remoteDirectory: '//opt//docker', remoteDirectorySDF: false, removePrefix: '', sourceFiles: 'Dockerfile')], usePromotionTimestamp: false, useWorkspaceInPromotion: false, verbose: false)])
		...
		stage ('Create the container with expose port 8087' ) {
			steps {
				...
				sshPublisher(publishers: [sshPublisherDesc(configName: 'docker-server-tomcat-tg', transfers: [sshTransfer(cleanRemote: false, excludes: '', execCommand: 'docker run -d -p 8087:8080 sasdevops/sasdevops_demo', execTimeout: 120000, flatten: false, makeEmptyDirs: false, noDefaultExcludes: false, patternSeparator: '[, ]+', remoteDirectory: '', remoteDirectorySDF: false, removePrefix: '', sourceFiles: '')], usePromotionTimestamp: false, useWorkspaceInPromotion: false, verbose: false)])
			}
